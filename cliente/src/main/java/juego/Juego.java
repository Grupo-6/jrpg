...
import main.java.cliente.HiloEscrituraJugador;
import main.java.cliente.HiloLecturaJugador;
...
public class Juego extends Canvas implements Runnable {
	...
	public void run() {
		...
		try {
			...
			System.err.println("Falla el cliente: "+jugador.getNombre());
			e.printStackTrace();
		}
		hiloEscritura = new Thread(new HiloEscrituraJugador(socket, jugador));
		hiloLectura= new Thread(new HiloLecturaJugador(socket,this));
		hiloEscritura.start();
		hiloLectura.start();
	...
	public void init() {
		...
		//level.addEntidad(new Elfo(level, 5, 5, input, "juan"));
